function plotLEfSe_pair(score,Enrich_lefse,tax_lefse, legend_ls,FaceColor)
if nargin<5
    FaceColor = defaultColor(length(legend_ls));
end
[idx1, ~] = AlignID(Enrich_lefse, legend_ls);
switch length(unique(idx1))
    case 2
        score = score*sign(median(score(idx1==2)));
    case 1
        score = score*sign(median(score(idx1==unique(idx1))));
end
[score,sortidx] = sort(score,'ascend');
tax_lefse = tax_lefse(sortidx);
Enrich_lefse = Enrich_lefse(sortidx);
idx1 = idx1(sortidx);
