function f_biplot(evects,scale,offset,sLabels,colorcode)
...
for j = 1:nRows
   ...
   cord = [thisVector(2,1)-thisVector(1,1),thisVector(2,2)-thisVector(1,2)];
   quiver(thisVector(1,1),thisVector(1,2),thisVector(2,1)-thisVector(1,1),thisVector(2,2)-thisVector(1,2),0, 'MaxHeadSize',0.15/norm(cord),'color','k')
   ...
   h = text(thisVector(2,1)+(deltaX*0.5),thisVector(2,2)+(deltaY*0.5),sLabels(j),'fontsize',16,'color',colorcode(j,:)); % label vectors
