function h = alluvialflow(data, left_labels, right_labels, chart_title, patch_colors)
...
for k_left = 1:size(data, 1) % for each row
    ...
    [bottom_curves_x, bottom_curves_y] = get_curves(0.15, patch_bottom_lefts, 0.85, patch_bottom_rights);
    [top_curves_x,    top_curves_y]    = get_curves(0.85, patch_top_rights,   0.15, patch_top_lefts);
    X = [ ...
        repmat([0.15; 0.15], 1, patches_per_left_category); % Top left, bottom left
        bottom_curves_x;
        repmat([0.85; 0.85], 1, patches_per_left_category); % Bottom right, top right
        top_curves_x
        ];
    ...
    Y = [ ...
        patch_top_lefts(1);
        patch_top_lefts(1);
        patch_bottom_lefts(end);
        patch_bottom_lefts(end);
        ];
    patch('XData', X, 'YData', Y, 'FaceColor', color, 'FaceAlpha', .8, 'EdgeColor', 'none');
    if k_left==1
        temp_top = patch_top_rights;
    end
    if k_left==size(data, 1)
        for k=1:patches_per_left_category
            color = patch_colors(mod(k,num_colors)+1, :);
            X = [ ...
                [0.85; 0.85]
                [1; 1]
                ];
...
Y = [ ...
    0
    right_category_points(end) + right_category_sizes(end);
    right_category_points(end) + right_category_sizes(end);
    0
    ];
patch('XData', X, 'YData', Y, 'FaceColor', [1 1 1], 'FaceAlpha', .3, 'EdgeColor', 'none');
