  ...
  function(expr_sub_t_df, rand_s, min_cl, ncl, groups) {
    ...
    by_class <-
      lapply(seq_along(groups), function(x) {
        cols_t <- cols
        cols_t <- cols_t[cols[, "class"] == groups[x],!(names(cols) %in% drops)]
        if (ncol(cols)==2){
          cols_t <- data.frame(c(cols_t))
          cn <- colnames(cols)
          rn <- row.names(cols)
          colnames(cols_t) <- cn[!(names(cols) %in% drops)]
          row.names(cols_t) <- rn[cols[, "class"] == groups[x]]
        }
        cols_t
      })
...
filterKruskal <- function(expr, group, p.value) {
  ...
  if (sum(kw.sub)!=1){
    ...
    tmp<-as.data.frame(expr[kw.sub,])
    rn <-row.names(expr)
    cn <- colnames(expr)
    tmp_t <-t(tmp)
    rownames(tmp_t) <- rn[kw.sub] 
    colnames(tmp_t) <- cn
    expr <- tmp_t
  }
}
...
lefser_m <-
           ...
           trim.names = FALSE)
  {
    ...
    if  (ncol(expr_sub_t_df)==2){
      raw_lda_scores <- mean(eff_size_mat)
      }
    else{
    ...
    if  (ncol(expr_sub_t_df)==2){
      rn <-row.names(expr_sub)
      scores_df <- data.frame(Names = rn,
...
D<-readMat("data3lefse_temp.mat");
x<-D$x;
y<-D$y;

se1 <- SummarizedExperiment(x)

se1$class <- array(y)
res_group <- lefser_m(se1, groupCol = "class")
writeMat("lefse_ana_result_tmp.mat",res_group=res_group)

#library(lefser)
#data(zeller14)
#zeller14 <- zeller14[, zeller14$study_condition != "adenoma"]
#res <- lefser_m(zeller14, groupCol = "study_condition", blockCol = "age_category")
#print(head(res))